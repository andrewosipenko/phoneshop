...
public class JdbcProductDao implements PhoneDao {
    ...
    private static final String SQL_FOR_ADDING_PHONE =
    ...
    private static final String SQL_FOR_GETTING_COLORS_BY_PHONE_ID =
                    "select * from phone2color where phone2color.phoneId = ?";
    @Resource
    ...
    private void setColorsForPhone(Phone phone) {
        ...
        jdbcTemplate.query(SQL_FOR_GETTING_COLORS_BY_PHONE_ID,
                        (resultSet, i) -> phone.getColors()
                                .add(colors.get(resultSet.getLong("colorId"))), phone.getId());
    }
    ...
    private void bindPhoneAndColor(Phone phone) {
        if (!phone.getColors().equals(Collections.EMPTY_SET)) {
            for (Color color : phone.getColors()) {
                jdbcTemplate.update(SQL_FOR_BINDING_PHONE_AND_COLOR,
